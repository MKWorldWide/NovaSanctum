...
export default function ResearchSearch() {
  ...
  const filtered = useMemo(() => {
    ...
    const pubs = (data.publications || [])
      .filter((p: any) =>
        categoryLower ? (p.category || '').toLowerCase().includes(categoryLower) : true
      )
      .filter((p: any) =>
        inRange(
          p.publicationDate
            ? new Date(p.publicationDate).getFullYear()
            : (p.year as number | undefined)
        )
      );

    const pats = (data.patents || []).filter((p: any) =>
      categoryLower ? (p.category || '').toLowerCase().includes(categoryLower) : true
    );

    const facs = (data.facilities || []).filter((f: any) =>
      countryLower ? (f.country || '').toLowerCase().includes(countryLower) : true
    );

    const inst = (data.institutes || []).filter((i: any) =>
      countryLower ? (i.country || '').toLowerCase().includes(countryLower) : true
    );
    ...
    const byImpact = (arr: any[]) =>
      [...arr].sort(
        (a, b) =>
          ((b.impactFactor || b.citations || 0) as number) -
          ((a.impactFactor || a.citations || 0) as number)
      );

    let pubsSorted = pubs;
    if (sortBy === 'date')
      pubsSorted = byDate(pubs, (p: any) =>
        p.publicationDate ? new Date(p.publicationDate) : undefined
      );
    if (sortBy === 'impact') pubsSorted = byImpact(pubs);

    return {
      publications: pubsSorted,
      patents: pats,
      facilities: facs,
      institutes: inst,
      laboratories: labs,
    ...
    <div className="mx-auto max-w-7xl px-6 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">
          Discover open educational materials while NovaSanctum transitions legacy datasets to fully
          mission-aligned resource catalogs.
        </p>
        <div className="mt-6 relative">
          <input
            ...
            placeholder="e.g., introductory statistics, web accessibility, python basics"
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 text-sm text-gray-600 md:grid-cols-2 lg:grid-cols-3">
          ...
          <input
            ...
            placeholder="Country filter (e.g., USA, UK)"
            value={country}
            onChange={e => setCountry(e.target.value)}
          />
          <input
            className="rounded border px-2 py-1"
            placeholder="Category filter (e.g., solar, battery)"
            value={category}
            onChange={e => setCategory(e.target.value)}
          />
          <input
            className="rounded border px-2 py-1"
            placeholder="Domain filter (comma-separated)"
            value={domainFilter}
        ...
        {loading && <div className="mt-4 text-sm text-gray-500">Searching “{debounced}”...</div>}
      ...
      {!loading && data && (
        <div>
          {(data as any).scholarly && (
            <Section
              ...
              items={(() => {
                ...
                (s.crossref || [])
                  .slice(0, 5)
                  .forEach((i: any) => x.push({ ...i, provider: 'Crossref' }));
                (s.arxiv || [])
                  .slice(0, 5)
                  .forEach((i: any) => x.push({ ...i, provider: 'arXiv' }));
                (s.pubmed || [])
                  .slice(0, 5)
                  .forEach((i: any) => x.push({ ...i, provider: 'PubMed' }));
                (s.semanticScholar || [])
                  .slice(0, 5)
                  .forEach((i: any) => x.push({ ...i, provider: 'Semantic Scholar' }));
                return x;
              })()}
              render={(it: any) => (
                <div>
                  ...
                  <div className="mt-1 text-xs text-gray-500">
                    {[it.venue || it.journal, it.year, it.provider].filter(Boolean).join(' • ')}
                  </div>
                  {it.doi && (
                    <a
                      className="mt-2 mr-2 inline-block text-sm text-indigo-600 hover:underline"
                  ...
                  <button
                    className="mt-2 ml-2 inline-block text-sm text-gray-700 hover:underline"
                    onClick={() => setDetails({ type: 'scholarly', data: it })}
                  >
                    Details
                  </button>
                </div>
            ...
            render={(p: any) => (
              <div>
                ...
                <button
                  ...
                  onClick={() => setDetails({ type: 'publication', data: p })}
                >
                  Details
                </button>
              </div>
          ...
          <Section
            title="Technical References (Legacy Data)"
            ...
            render={(pt: Patent) => (
              <div>
                ...
                <button
                  className="mt-2 inline-block text-sm text-gray-700 hover:underline"
                  onClick={() => setDetails({ type: 'patent', data: pt })}
                >
                  Details
                </button>
              </div>
          ...
          <Section
            ...
            render={(f: Facility) => (
              <div>
                ...
                <button
                  className="mt-2 ml-2 inline-block text-sm text-gray-700 hover:underline"
                  onClick={() => setDetails({ type: 'facility', data: f })}
                >
                  Details
                </button>
              </div>
          ...
          <Section
            title="Institutions (Legacy Data)"
            ...
            render={(i: Institute) => (
              <div>
                ...
                <button
                  className="mt-2 ml-2 inline-block text-sm text-gray-700 hover:underline"
                  onClick={() => setDetails({ type: 'institute', data: i })}
                >
                  Details
                </button>
              </div>
          ...
          <Section
            ...
            render={(l: Lab) => (
              <div>
                ...
                <button
                  className="mt-2 inline-block text-sm text-gray-700 hover:underline"
                  onClick={() => setDetails({ type: 'lab', data: l })}
                >
                  Details
                </button>
              </div>
      ...
      <Modal
        ...
        title={details?.type ? String(details.type).toUpperCase() : 'Details'}
      >
