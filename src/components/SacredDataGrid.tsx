...
export const SacredDataGrid = <T extends object>({
  ...
  const handleFilterChange = useCallback(
    (column: string, value: string, operator: Filter['operator']) => {
      setFilters(current => {
        const existing = current.find(f => f.column === column);
        if (existing) {
          return current.map(f => (f.column === column ? { ...f, value, operator } : f));
        }
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      {error && <SacredAlert type="error" title="Error" message={error} onClose={onErrorDismiss} />}
