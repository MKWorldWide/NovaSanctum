...
import { ScienceInstitute } from '../../../src/types/scientific-institutes.types';

// Test data
const TEST_INSTITUTE: Omit<ScienceInstitute, 'id' | 'createdAt' | 'updatedAt'> = {
  name: 'Test Science Institute',
  type: 'university',
  country: 'Testland',
  location: {
    city: 'Testville',
  ...
  ranking: 1,
  nobelPrizes: 5,
  researchAreas: ['Quantum Physics', 'AI'],
  internationalPartners: [],
  facilities: [],
  publications: 5000,
  patents: 100,
  status: 'active',
  website: 'https://testinstitute.edu',
  notes: 'Test institution for unit testing'
};
...
describe('TopScienceInstitutes Service', () => {
    ...
    it('should search institutes by specialty', () => {
      ...
      if (results.length > 0) {
        expect(results.every(i => 
          i.specialties.some((s: string) => s.includes(specialty))
        )).to.be.true;
      }
    });
  });
  ...
  describe('Research Laboratories', () => {
    ...
    it('should search laboratories by specialty', () => {
      ...
      expect(results).to.be.an('array');
      if (results.length > 0) {
        expect(results.every(lab => 
          lab.specialties.some((s: string) => s.includes(specialty))
        )).to.be.true;
      }
    });
  });
  ...
  describe('Data Integrity', () => {
    it('should have valid data in all institutes', () => {
      ...
      for (const institute of institutes) {
        ...
        expect(institute).to.have.property('type').that.is.oneOf(['university', 'research_center', 'laboratory', 'institute', 'foundation']);
        ...
        expect(institute).to.have.property('status').that.is.oneOf(['active', 'inactive', 'expanding']);
